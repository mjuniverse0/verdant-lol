...
public class CarCheats : CheatsUtilities, ICheatsBase, IRevertBase
{
    ...
    public async Task<bool> CheatLocalPlayer()
    {
        ...
        var asm = new byte[]
        {
            ...
            // Race check: original code skipped velocity/brake/wheelspeed when [race+0x3D8]==1.0f (race mode).
            // NOP the jz so velocity hack works in races too (0x0F 0x84 = jz rel32 → 6x NOP).
            0x8D, 0x80, 0xD8, 0x03, 0x00, 0x00, 0x81, 0x38, 0x00, 0x00, 0x80, 0x3F, 0x90, 0x90, 0x90, 0x90, 0x90,
            0x90, 0xF3, 0x0F, 0x10, 0x4F, 0x20, 0xF3, 0x0F, 0x10, 0x57, 0x28, 0xF3, 0x0F, 0x59, 0xC9, 0xF3, 0x0F,
    ...
    public async Task<bool> CheatAccel()
    {
        ...
        var asm = new byte[]
        {
            ...
            raceBytes[4], raceBytes[5], raceBytes[6], raceBytes[7], 0x48, 0x8B, 0x00, 0x48, 0x8B, 0x40, 0x50, 0x48,
            // Race check: original code skipped accel hook when [race+0x3D8]==1.0f (race mode).
            // NOP the jz so acceleration modifier works in races too.
            0x8D, 0x80, 0xD8, 0x03, 0x00, 0x00, 0x81, 0x38, 0x00, 0x00, 0x80, 0x3F, 0x90, 0x90, 0x90, 0x90, 0x90,
            0x90, 0x48, 0xB8, localPlayerAddr[0], localPlayerAddr[1], localPlayerAddr[2], localPlayerAddr[3],
